...
@
\caption{Partial rank correlation coefficient analysis \cite{pujol_sensitivity:_2014,saltelli_sensitivity_2009} of the effects of ideology, market completeness, number of edges in social network, number of traders in market, risk taking propensities, segregation measure of the social network, and the true climate model on the convergence of traders' beliefs. Lines are bootstrapped 95\% confidence intervals.}
\label{fig:pc_sa}
\end{figure}
...
@
\caption{Density of \Sexpr{sample_count} convergence of belief model outcomes with \Sexpr{formatC(sample_count, big.mark=',')} Latin Hypercube Sampled input parameter sets. \Sexpr{formatC(sum(ifelse(d$true.model=="Human-Induced", 1, 0)), big.mark=',')} of the Latin Hypercube Sampled input parameter sets have ``human-induced'' climate change as the true model and \Sexpr{formatC(sum(ifelse(d$true.model=="Natural Change", 1, 0)), big.mark=',')} have ``natural change'' climate change as the true model. Dashed lines are the medians of the two densities (\Sexpr{cdat$outcome_summary[1]}, \Sexpr{cdat$outcome_summary[2]}).}
\label{fig:hist}
\end{figure}
