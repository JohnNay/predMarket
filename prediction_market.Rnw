...
\begin{figure}
\centering
<<temporal, eval=TRUE, echo=FALSE, message=FALSE, cache=TRUE, fig.width=10, fig.height=6>>=
sample_count <- 1000
load("output/average_convergence.Rda")
dat <- average_convergence[average_convergence$true_mod != "Both", ]
dat$true_mod <- factor(dat$true_mod, levels = c("Natural", "Human-Induced"), 
                       labels = c("Natural Change", "Human-Induced"))
library(ggplot2)
ggplot(data= dat, aes(x= trading_seq, y=avg, color = true_mod)) +
  geom_smooth(method = "loess") + 
  ggtitle("Average Convergence Over Trading Sequences") +
  xlab("Trading Sequences") + 
  ylab(paste0("Average Convergence (n = ", formatC(sample_count, big.mark = ','), ")")) + 
