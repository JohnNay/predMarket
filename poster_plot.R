...
p <- ggplot(data=plot_data, aes(x= trading_seq * sequence.years + 2014, y=(convergence + 1) * 50, color = true_mod)) +
  geom_point(position = position_jitter(w = 0.2 * sequence.years, h = 0), alpha = 0.1) + geom_smooth(size=1, fill=NA) +
...
if (TRUE) {
  pdf("output/jg_timeseries_future_box.pdf", width=8, height=18)
  print(p)
  dev.off()
}
