library(dplyr)
library(tidyr)
# replace ggplot2 with jgplot2 that has my enhancement for outlier.alpha on geom_boxplot.
# library(ggplot2)
library(jgplot2) # devtools::install_github('jonathan-g/jgplot2', ref = 'jgplot2')
...
future_box <- function(){
  ...
  ggplot(data=complete_data, aes(x= trading_seq * sequence.years + 2014, y=(convergence + 1) * 50, color = true_mod)) +
    ...
    jgplot2::geom_boxplot(data = plot_data,
...
history_box <- function(){
  ...
  ggplot(data=complete_data, aes(x= trading_seq * sequence.years + start, y=(convergence + 1) * 50,  color = true_mod)) +
    geom_point(data = initial_convergence, size = 2) +
    jgplot2::geom_boxplot(data = plot_data, aes(group = floor(trading_seq * sequence.years + start * (true_mod == "SlowTSI"))),
