data {
  ...
  int<lower=0,upper=T_future> horizon;
...
generated quantities {
  ...
  real y_horizon;
  {
    ...
    y_future <- rep_vector(0, T_future);
    yy <- append_row(y, y_future);
  ...
  y_future <- tail(yy,T_future) + m * x_future + b;
  }
  if (horizon > 0)
    y_horizon <- y_future[horizon];
  else
    y_horizon <- 0;
}
